...
$.each(opts.after,function(i,o){o.apply(next,[curr,next,opts,fwd])});queueNext(opts)};if(opts.nextSlide!=opts.currSlide){opts.busy=1;$.fn.cycle.custom(curr,next,opts,after)}var roll=opts.nextSlide+1==els.length;opts.nextSlide=roll?0:opts.nextSlide+1;opts.currSlide=roll?els.length-1:opts.nextSlide-1}else queueNext(opts);function queueNext(opts){if(opts.timeout)p.cycleTimeout=setTimeout(function(){go(els,opts,0,!opts.rev)},opts.timeout)}}function advance(els,opts,val){var p=els[0].parentNode,timeout=
p.cycleTimeout;if(timeout){clearTimeout(timeout);p.cycleTimeout=0}opts.nextSlide=opts.currSlide+val;if(opts.nextSlide<0)opts.nextSlide=els.length-1;else if(opts.nextSlide>=els.length)opts.nextSlide=0;go(els,opts,1,val>=0);return false}$.fn.cycle.custom=function(curr,next,opts,cb){var $l=$(curr),$n=$(next);$n.css(opts.cssBefore);var fn=function(){$n.animate(opts.animIn,opts.speedIn,opts.easeIn,cb)};$l.animate(opts.animOut,opts.speedOut,opts.easeOut,function(){$l.css(opts.cssAfter);if(!opts.sync)fn()});
if(opts.sync)fn()};$.fn.cycle.transitions={fade:function($cont,$slides,opts){$slides.not(":eq(0)").hide();opts.cssBefore={opacity:0,display:"block"};opts.cssAfter={display:"none"};opts.animOut={opacity:0};opts.animIn={opacity:1}},fadeout:function($cont,$slides,opts){opts.before.push(function(curr,next,opts,fwd){$(curr).css("zIndex",opts.slideCount+(fwd===true?1:0));$(next).css("zIndex",opts.slideCount+(fwd===true?0:1))});$slides.not(":eq(0)").hide();opts.cssBefore={opacity:1,display:"block",zIndex:1};
opts.cssAfter={display:"none",zIndex:0};opts.animOut={opacity:0};opts.animIn={opacity:1}}};$.fn.cycle.ver=function(){return ver};$.fn.cycle.defaults={animIn:{},animOut:{},fx:"fade",after:null,before:null,cssBefore:{},cssAfter:{},delay:0,fit:0,height:"auto",metaAttr:"cycle",next:null,pause:false,prev:null,speed:1E3,slideExpr:null,sync:true,timeout:4E3}})(jQuery);


$(function(){
	$('.slider').each(function(){
		var carousel = $(this);
		var prev = $(this).parent().find('.l');
		var next = $(this).parent().find('.r');
		carousel.cycle({fx:'fade', speed: 'fast', timeout: 0, next: next, prev: prev});
	})

	$('.d, .widget .a').each(function(){
		$(this).click(function(){
			var target = $(this).attr('data-target');
			$('body,html').animate({ scrollTop: $('.'+target).offset().top - 170 }, 800);
		})
	})

    var headerHeight = 91;
    var submenuLineHeight = 14;
    var submenuPadding = 26;

	$('.header .nav > li.hsub').hover(function(){
        var submenuFloors = $(this).find('.submenu-column').eq(0).find('> li').length;
        $( "header.header" ).stop(true,true).animate({
            height: (headerHeight + submenuFloors * submenuLineHeight + submenuPadding) + "px"
        }, 'fast');
		$(this).find('.submenu-wrap').fadeIn('fast');
	}, function(){
        $( "header.header" ).stop(true,true).animate({
            height: headerHeight + "px"
        }, 'fast');
		$(this).find('.submenu-wrap').fadeOut('fast');
	})
})
